...
import './platform.css';
import { ExecutionChoice, type ExecutionMode } from './ExecutionChoice';
...
import { defaultSources, readTaskDraft, taskExamples, type TaskDraft } from './platform-draft';
import { platformHash, readPlatformHash, usePlatformRoute } from './hash-route';

type Goal = { spec?: { delivery?: DeliveryConfig }; toolPayments?: any[]; settlement?: any; id: string; input: { goal: string; kind: 'analysis' | 'research'; reward: string; sourceUrls?: string[] }; status: string; createdAt: string; taskId?: string; task?: any; result?: any; evidence?: any; verificationHistory?: any[]; events?: any[]; error?: string; publication?: { id: string; status: string; error?: string }; audit?: { verdict: string; reason: string } };
...
export default function PlatformApp() {
  ...
  const [execution, setExecution] = useState<ExecutionMode | undefined>(undefined);
  const [draft] = useState(readTaskDraft); const goalInput = useRef<HTMLTextAreaElement>(null);
  const [goal, setGoal] = useState(draft.goal); const [kind, setKind] = useState<TaskDraft['kind']>(draft.kind); const [sources, setSources] = useState(draft.sources); const [reward, setReward] = useState(draft.reward); const [amount, setAmount] = useState('0.5'); const [fromBlock, setFromBlock] = useState(draft.fromBlock); const [toBlock, setToBlock] = useState(draft.toBlock);
  ...
  useEffect(() => { try { localStorage.setItem('worknet-platform-draft', JSON.stringify({ goal, kind, sources, reward, fromBlock, toBlock })); localStorage.setItem('worknet-goal-draft', goal); } catch { /* The form remains usable when browser storage is unavailable. */ } }, [goal, kind, sources, reward, fromBlock, toBlock]);
  ...
  const plan = () => action('plan', async () => {
    ...
    if (!execution) throw new Error('请选择开放接单或平台执行器，再生成计划。');
    const input = { goal, kind, execution, reward: parseUnits(reward, 6).toString(), ...(kind === 'research' ? { sourceUrls: sources.split(/\s+/).filter(Boolean) } : { ...(fromBlock ? { fromBlock } : {}), ...(toBlock ? { toBlock } : {}) }) };
    ...
    const created = await api<Goal>('plans', { id: planIntent.current.id, ...input, ...(delivery ? {delivery} : {}) }); await refresh(); location.hash = platformHash({ goal: created.id }); toast.success('计划已准备好。确认发布后才会托管任务奖励。');
  });
  ...
  const navigate = (next: View) => { location.hash = platformHash({ view: next }); };
  const openGoal = (id: string) => { location.hash = platformHash({ view, goal: id }); };
  return <div className="workspace-shell platform-shell">
    <aside className="rail"><a href="/" aria-label={tr("Worknet 首页")}><Brand/></a><div className="platform-identity"><span className="identity-mark"><Icon name="wallet"/></span><div><strong>{owner ? short(owner) : tr("你的独立账户")}</strong><small>{owner ? tr("个人账户") : tr("使用通行密钥登录")}</small></div></div><button className="button primary new-goal" onClick={() => navigate('home')}><Icon name="plus" size={17}/>{tr("发布新任务")}</button><nav aria-label={tr("主导航")}>{([['home','goal',tr("我的任务")],['deliveries','file',tr("交付文档")],['budget','wallet',tr("预算与账户")],['taker','grid',tr("接单")]] as const).map(([id,icon,label]) => <button key={id} className={view === id ? 'active' : ''} onClick={() => navigate(id)}><Icon name={icon}/>{label}</button>)}</nav><div className="rail-bottom"><div className="connection-card"><span className={`status-dot ${online ? 'online' : ''}`}/><div>{online ? tr("平台执行服务在线") : tr("正在检查服务")}<small>{tr("关闭页面后任务仍可继续")}</small></div></div></div></aside>
    ...
    {view === 'taker' ? <TakerPanel key={owner??'anonymous'} owner={owner} account={account} wallet={wallet} connect={connect} createAgentAccount={()=>{setLoginError('');setLoginOpen(true);return loginMera('create');}} restoreAgentAccount={()=>{setLoginError('');setLoginOpen(true);return loginMera('signin');}} config={config} target={runTarget}/> : current ? <><button className="back-button" onClick={() => { location.hash = platformHash({ view }); }}>{tr("返回我的任务")}</button><div className="detail-heading"><span className={`pill ${current.status}`}>{tr(status(current))}</span><h1>{current.input.goal}</h1><p>{tr("任务奖励")}<Money amount={current.input.reward}/> test USDC</p></div>
      {!current.taskId ? <div className="plan-layout"><article className="plan-paper"><h2>{tr("这次任务的交付约定")}</h2>{(current.publication?.error || current.error) && <p className="document-warning" role="status">{tr(current.publication?.error || current.error)}</p>}{publicationPending && <p className="document-warning">{tr("发布请求已收到，正在等待后台确认；无需重复提交。")}</p>}<div className="platform-brief"><Icon name={current.input.kind === 'research' ? 'spark' : 'grid'} size={32}/><div><h3>{current.input.kind === 'research' ? tr("有来源引用的研究简报") : tr("可独立复算的转账分析")}</h3><p>{current.input.kind === 'research' ? tr("阅读指定资料，形成回答目标的简报；重新抓取来源并由独立 Judge 审核。") : tr("完整查询指定区间的测试 USDC Transfer，交付次数与原始金额，并独立复算。")}</p></div></div>{current.input.sourceUrls?.map(uri => <p key={uri}><a href={safeSource(uri)} target="_blank" rel="noreferrer">{uri}</a></p>)}<p>{(current.input as any).execution === 'market' ? tr("这项任务将开放接单，平台执行器不参与领取。") : tr("这项任务由平台执行器处理。")}{tr("任务与交付公开保存。执行期限 1 小时，提交后审核窗口 10 分钟。审核超时可能付款，不等于验证通过。")}</p></article><aside className="plan-confirm"><Icon name="wallet" size={26}/><h2>{tr("确认并发布")}</h2><p className="execution-confirm">{tr("发布方式：")}<strong>{(current.input as any).execution === 'market' ? tr("开放接单") : tr("平台执行器")}</strong></p><div className="plan-price"><Money amount={current.input.reward}/><span>test USDC</span></div><p>{tr("奖励将从你的独立预算托管到任务合约。")}</p>{current.input.kind === 'research' && config?.quorum && <QuorumTerms/>}<p>{tr("如需钱包交易确认，手续费以钱包显示为准；完成后自动继续发布，无需再次充值 USDC。")}</p>{needsPaymentConfirmation && <details className="recharge-options"><summary>{tr("本次钱包确认的付款范围")}</summary><p>{tr("24 小时内累计最多")}<Money amount={renewalLimit}/>{tr("，单笔最多")}<Money amount={BigInt(renewalLimit) < 200000n ? renewalLimit : '200000'}/> test USDC。</p>{hasActiveTasks && <p>{tr("你还有进行中的任务；更新授权后，这些任务可能需要你手动审核。")}</p>}</details>}<button className="button primary full-width" disabled={!!busy || publicationPending || !!budgetIssue} onClick={() => void launch(current)}>{busy === 'launch' || publicationPending ? tr(paymentProgress) || tr("正在确认发布…") : current.publication?.status === 'failed' ? tr("重试发布") : tr("确认并发布")}</button>{budgetIssue && <><p role="status">{tr(budgetIssue)}</p><button className="text-button full-width" onClick={() => navigate('budget')}>{tr("充值")}</button><button className="text-button full-width" disabled={!!busy} onClick={() => void action('refresh-budget', refresh)}>{tr("刷新预算状态")}</button></>}</aside></div> : <>
      <MarketRules/>{current.input.kind === 'research' && config?.quorum && <QuorumTerms/>}<QuorumPanel ended={Number(current.task?.status)>=3} evidence={current.evidence} settlement={current.settlement}/><RecoveryHint item={current} requester/><div className="execution-progress">{[tr("等待接单"),tr("执行"),tr("审核"),tr("交付")].map((label,i) => <div key={label} className={Number(current.task?.status ?? -1)<=3 && Number(current.task?.status ?? -1) >= i ? 'done' : ''}><span>{i + 1}</span><b>{label}</b></div>)}</div>{current.error && <p className="document-warning">{tr(current.error)}</p>}
      {current.result && isPrivateOutput(current.result.output) ? <PrivateDelivery key={`${owner}:${current.taskId}:${current.result.attempt}`} result={current.result} delivery={current.spec?.delivery} canUnlock/> : current.result ? <article className="deliverable"><div className="deliverable-head"><Icon name="file"/><h2>{tr("任务交付")}</h2><span className={`pill ${current.evidence?.verdict === 'accept' && current.audit?.verdict !== 'reject' ? 'completed' : 'attention'}`}>{current.evidence?.verdict === 'scored' ? tr("已评定完成度") : current.audit?.verdict === 'reject' ? tr("复核未通过") : current.evidence?.verdict === 'accept' ? tr("独立检查通过") : tr("尚未通过检查")}</span></div><div className="result-document">{current.input.kind === 'analysis' ? <div className="analysis-result"><div><span>{tr("转账次数")}</span><strong>{current.result.output.eventCount}</strong></div><div><span>{tr("转账总额")}</span><strong><Money amount={current.result.output.totalAmountBaseUnits}/><small>test USDC</small></strong></div></div> : <><p className="research-summary">{current.result.output.summary}</p>{current.result.output.findings?.map((f: any,i: number) => <section className="finding" key={i}><h3>{f.title}</h3><p>{f.claim}</p><blockquote>{f.quote}</blockquote><a href={safeSource(f.sourceUri)} target="_blank" rel="noreferrer">{tr("查看来源")}</a></section>)}</>}</div></article> : <div className="empty-state"><Icon name="clock" size={30}/><h3>{Number(current.task?.status)>=4?tr("任务已结束"):current.task?.status===3?tr("任务已结算，等待记录同步"):tr("任务正在平台上推进")}</h3><p>{Number(current.task?.status)>=4?tr("本任务不会继续执行，请核对退款与预算记录。"):tr("你可以离开页面，稍后回到这里阅读交付。")}</p></div>}
    ...
    </> : view === 'home' ? <><div className="home-heading"><span className="heading-symbol"><Icon name="goal" size={30}/></span><h1>{tr("让目标，成为交付。")}</h1><p>{tr("用自己的预算发布任务。Agent 执行，结果独立检查，奖励按约定结算。")}</p></div>{!owner && <div className="platform-welcome"><div><h2>{tr("从你的通行密钥开始")}</h2><p>{tr("登录签名仅用于确认身份。发布前，你会看到工作内容、奖励和验收方式。")}</p></div><button className="button primary" disabled={!!busy} onClick={() => void connect()}>{tr("登录或创建账户")}</button></div>}
      {owner && <BudgetReminder account={account} onSetup={()=>navigate('budget')}/>}
      <form className="goal-composer" onInvalid={e => (e.target as HTMLElement).closest('details')?.setAttribute('open', '')} onSubmit={e => { e.preventDefault(); if (owner) void plan(); else void connect(); }}>
        <label className="composer-label" htmlFor="platform-goal">{tr("你想完成什么？")}</label>
        <textarea ref={goalInput} id="platform-goal" value={goal} onChange={e => setGoal(e.target.value)} minLength={8} maxLength={2000} required placeholder={tr("描述要完成的任务、期望交付物和验收要求，也可以从下方示例开始。")}/>
        {kind === 'research' && config?.quorum && <QuorumTerms/>}<ExecutionChoice value={execution} onChange={setExecution}/>
        <div className="private-delivery-option"><label><input type="checkbox" checked={privateDelivery && !legacyWalletMode && !!config?.privacy?.reviewPublicKey} disabled={legacyWalletMode||!config?.privacy?.reviewPublicKey} onChange={e=>setPrivateDelivery(e.target.checked)}/>{tr("交付物使用通行密钥加密")}</label><p>{config?.privacy?.reviewPublicKey ? tr("发布时创建此任务的独立钥匙。执行器和审核服务可读取处理内容，公开存储仅保留加密交付。") : tr("当前环境尚未启用私有交付，本次交付将公开保存。")}</p></div><details className="source-settings"><summary>{tr("交付设置与奖励")}<Icon name="chevron" size={14}/></summary><div className="platform-fields">
          <label>{tr("交付格式")}<select value={kind} onChange={e => setKind(e.target.value as TaskDraft['kind'])} aria-describedby="platform-format-help"><option value="research">{tr("附来源的研究简报")}</option><option value="analysis">{tr("USDC 转账统计")}</option></select></label>
          <p id="platform-format-help">{tr("当前支持 Monad 官方文档研究与测试 USDC 转账统计，生成计划后可核对交付约定。")}</p>
          {kind === 'research' ? <label>{tr("参考资料（每行一条，最多 3 条 Monad 官方文档链接）")}<textarea value={sources} onChange={e => setSources(e.target.value)} required/></label> : <div className="field-pair"><label>{tr("起始区块（留空使用最近区间）")}<input value={fromBlock} onChange={e => setFromBlock(e.target.value)} inputMode="numeric"/></label><label>{tr("结束区块")}<input value={toBlock} onChange={e => setToBlock(e.target.value)} inputMode="numeric"/></label></div>}
          <label>{tr("任务奖励（0.01–0.20 test USDC）")}<input value={reward} onChange={e => setReward(e.target.value)} inputMode="decimal"/></label>
        </div></details>
        {kind === 'research' && config && !config.modelAvailable && <p className="composer-availability" role="status">{tr("研究执行服务暂不可用，草稿已保留，请稍后再试。")}</p>}
        <div className="composer-footer"><span><Icon name="check" size={15}/>{tr("先审阅计划，确认后再发布")}</span><button className="button primary" disabled={!!busy || goal.trim().length < 8 || !execution || (kind === 'research' && config && !config.modelAvailable)}>{busy === 'plan' ? tr("正在准备…") : owner ? tr("生成任务计划") : tr("登录并继续")}<Icon name="arrow" size={17}/></button></div>
      </form>
      <section className="starter-section platform-starters" aria-labelledby="platform-starters-title">
        <div className="section-heading"><h2 id="platform-starters-title">{tr("从这些目标开始")}</h2><span>{tr("点击填入，修改后再生成计划")}</span></div>
        <div className="starter-grid">{taskExamples.map((item, i) => <button type="button" key={tr(item.title)} className="starter" onClick={() => { setGoal(item.goal); setKind(item.kind); setSources(item.sources.length ? item.sources.join('\n') : defaultSources); setFromBlock(''); setToBlock(''); goalInput.current?.focus(); }}><span className={`starter-icon tone-${i}`}><Icon name={item.icon}/></span><strong>{tr(item.title)}</strong><Icon name="arrow" size={17}/></button>)}</div>
      </section>
