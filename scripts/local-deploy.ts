...
const judgeAccounts = [5, 6, 7].map(index => mnemonicToAccount(mnemonic, { addressIndex: index }));
const token = await deploy('MockUSDC'); const manager = await deploy('TaskManager', [token, judgeAccounts.map(judge => judge.address), 2]); const vault = await deploy('RequesterVault', [owner.address, manager, token]);
...
const judgeUrls = (process.env.JUDGE_URLS ?? 'http://127.0.0.1:8791,http://127.0.0.1:8792,http://127.0.0.1:8793').split(',').map(value => value.trim());
...
await writeFile(output, JSON.stringify({
  ...
  judgeUrls, judges: { accounts: judgeAccounts.map(judge => judge.address), threshold: 2 },
...
console.log(JSON.stringify({ mode: 'local-demo', config: output, token, manager, vault, judges: judgeAccounts.map(judge => judge.address), judgeUrls }));
