...
export async function privateJson<T>(name: string, create: () => T): Promise<T> {
  await mkdir(platformDirectory, { recursive: true, mode: 0o700 });
  try { return JSON.parse(await readFile(`${platformDirectory}/${name}`, 'utf8')) as T; }
  catch (error) { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error; const value = create(); await persistPrivate(name, value); return value; }
}
export async function persistPrivate(name: string, value: unknown) {
  await mkdir(platformDirectory, { recursive: true, mode: 0o700 });
  const file = `${platformDirectory}/${name}`;
...
export class JournalWallet {
  ...
  constructor(key: Hex, readonly name: string) { this.account = privateKeyToAccount(key); this.wallet = createWalletClient({ account: this.account, chain, transport: http(rpcUrl) }); }
  async send(id: string, call: { to?: Address; data?: Hex; value?: bigint; authorizationList?: SignedAuthorization[]; gas?: bigint }) {
    const entries = await privateJson<Record<string, Entry>>(`journal-${this.name}.json`, () => ({}));
    ...
    if (!entry) {
      ...
      await persistPrivate(`journal-${this.name}.json`, entries);
    }
    ...
    entry.status = receipt.status; entry.contractAddress = receipt.contractAddress ?? null; await persistPrivate(`journal-${this.name}.json`, entries);
