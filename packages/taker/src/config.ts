...
export const configPath=()=>resolve(process.env.WORKNET_TAKER_CONFIG??`${homedir()}/.config/worknet/taker.json`);
export async function loadConfig():Promise<LocalConfig>{return JSON.parse(await readFile(configPath(),'utf8'));}
...
export async function configuredClient(signal?:AbortSignal):Promise<TakerClient>{const config=await loadConfig();if(config.executionKey){const {AgentClient}=await import('./agent.js');return new AgentClient(config.origin,config.token,config.executionKey,config.id,configPath(),signal);}return new TakerClient(config.origin,config.token,signal);}
