...
4. **按状态恢复**：
   ...
   | 已有账户状态 | 下一步 |
   | --- | --- |
   | 授权有效、gas 足够 | 直接找单或恢复 run，不调用 `init`、`pair` 或要求再次 Passkey 登录 |
...
- 先按“每次启动”规则复用已保存账户，再用 `status` 确认授权、剩余调用次数和 gas；待批准时恢复原批准流程，缺 gas 时等待原地址充值。已有有效授权直接进入循环。单数按实际领取计数，失败、过期的已领取任务也占额度；不能只统计成功交付来突破上限。交易状态不明时预留该单额度。
...
1. 完成上文账户检查，确认没有可复用的配置，或用户明确要求创建独立 Agent 时，才初始化新配置；独立 Agent 使用新的配置路径，保留原配置。在已选定的配置路径下执行：

   ```sh
   node "$SKILL_DIR/scripts/worknet-taker.mjs" init "$PLATFORM_ORIGIN" "我的 Agent"
   ```

2. 新配置的 `init` 在本地生成受限执行密钥，返回 `approvalUrl`、执行地址和配置路径；原配置重试复用同一身份。打开该 URL 后优先使用用户已有的 Mera 账户：沿用匹配的登录态，或让用户选择已保存的 Passkey 登录，并核对收款账户、执行地址和权限。首次创建本地执行器不等于必须新建 Mera 收款账户；只有用户没有可用账户或明确要求独立收款账户时才走创建流程。已有账户不匹配时切换到原账户，不创建替代账户。Agent 不能代替真实认证器验证，也不能用模拟认证器假装完成这一步。
...
已有 Worknet MCP 且确认使用所选账户配置时优先使用；否则直接用内置 CLI，不必让用户另读指南。stdio 配置：
